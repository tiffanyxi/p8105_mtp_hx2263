...
### Using these data, explore the hypothesis that this participant became more active over time.
...
total_activity_over_day %>% 
  group_by(day) %>% 
  summarise(dow_total = mean(total)) %>% 
  ...
  labs(
    title = "Total Activity Over the Day",
    x = "Day of the Week",
    y = "Day of the Week Effects on Activity",
    caption = "Data from p8105_mtp_data"
  )
```
...
am_data_tidy %>% 
  ...
  theme_bw()+ 
  labs(
    title = "Activity Over the Time",
    x = "Time of a Day",
    y = "Day of the Week Effects on Activity in Addition to the Effect of Time",
    caption = "Data from p8105_mtp_data"
  )
...
*Comment*:
Day of the week does affect activity in addition to the effect of time. Curve of thursday, friday,and sunday have larger flu
